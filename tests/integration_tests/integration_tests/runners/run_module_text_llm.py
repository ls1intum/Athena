...
@pytest.fixture
def run_module_text_llm(monkeypatch):
    ...
    # secrets
    monkeypatch.setenv("SECRET", "integration test MODULE_TEXT_LLM_SECRET")
    ...
    module_cwd = os.path.join(current_cwd, "modules/text/module_text_llm")

    ON_POSIX = 'posix' in sys.builtin_module_names

    python_executable = os.path.join(module_cwd, ".venv", "bin", "python")
    process = subprocess.Popen(
        [python_executable, "-m", "module_text_llm"],
    ...
    if not ready:
        ...
        raise TimeoutError(f"Module Text LLM didn't start at time.\nError: {stderr_output}")
