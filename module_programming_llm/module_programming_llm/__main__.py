...
def process_incoming_feedback(
    exercise: Exercise, submission: Submission, feedbacks: List[GradedFeedback]
):
    logger.info(
...
@graded_feedback_provider
async def suggest_graded_feedback(
    exercise: Exercise, submission: Submission, module_config: Configuration
) -> List[GradedFeedback]:
    logger.info(
        "suggest_feedback: graded suggestions for submission %d of exercise %d were requested",
        submission.id,
        exercise.id,
    )
    return await generate_graded_suggestions_by_file(
        exercise, submission, module_config.graded_approach, module_config.debug
    )


@non_graded_feedback_provider
async def suggest_non_graded_feedback(
    exercise: Exercise, submission: Submission, module_config: Configuration
) -> List[NonGradedFeedback]:
    logger.info(
        "suggest_feedback: non graded suggestions for submission %d of exercise %d were requested",
        submission.id,
        exercise.id,
    )
    return await generate_non_graded_suggestions_by_file(
        exercise, submission, module_config.non_graded_approach, module_config.debug
    )
