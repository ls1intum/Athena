...
if os.environ.get("REPLICATE_API_TOKEN"):  # If Replicate is available
    available_models = {
        name: Replicate(model=model, model_kwargs={"temperature": 0.01})
        for name, model in replicate_models.items()
    }
else:
    logger.warning(
        "REPLICATE_API_TOKEN not found in environment variables. Replicate models are disabled."
    )

if available_models:
    logger.info("Available replicate models: %s", ", ".join(available_models.keys()))
    ...
    class ReplicateModelConfig(ModelConfig):
        ...
        model_name: ReplicateModel = Field(
            default=default_replicate_model,  # type: ignore
