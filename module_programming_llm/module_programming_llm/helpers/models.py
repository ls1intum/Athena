...
if OPENAI_API_TYPE == "azure":
    if "OPENAI_API_BASE" not in os.environ:
        raise Exception("OPENAI_API_TYPE=azure but OPENAI_API_BASE environment variable not set.")
    if "OPENAI_API_VERSION" not in os.environ:
        raise Exception("OPENAI_API_TYPE=azure but OPENAI_API_VERSION environment variable not set.")
    if "AZURE_DEPLOYMENT_NAME" not in os.environ:
        raise Exception("OPENAI_API_TYPE=azure but AZURE_DEPLOYMENT_NAME environment variable not set.")
    ...
    if AZURE_DEPLOYMENT_NAME not in deployment_ids:
        ...
        raise Exception(f"Deployment id '{AZURE_DEPLOYMENT_NAME}' not found, available deployments: {deployments}")
