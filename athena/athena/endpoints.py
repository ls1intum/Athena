...
def ungraded_feedback_provider(func: Union[
    Callable[[E, S], List[F]],
    Callable[[E, S], Coroutine[Any, Any, List[F]]],
    Callable[[E, S, C], List[F]],
    Callable[[E, S, C], Coroutine[Any, Any, List[F]]],
    ...
    @app.post("/ungraded_feedback_suggestions2", responses=module_responses)
    @with_meta
    async def wrapper(
            exercise: exercise_type,
            submission: submission_type,
...
@app.post("/ungraded_feedback_suggestions")
async def testing():
    system_message = "youre are a funny person"
    prompt = "Tell a joke with no bikes"
