 ...
 */
CREATE VIEW relevant_text_exercises AS
-- Select course exercises
SELECT DISTINCT e.id, c.id AS course_id, 0 as is_exam_exercise
FROM
    exercise e
...
SELECT DISTINCT e.id, c.id AS course_id, 1 as is_exam_exercise
FROM
    course c,
    exam ex,
    exercise_group eg,
...
SELECT
  JSON_OBJECT(
    ...
    'example_solution', e.example_solution,
