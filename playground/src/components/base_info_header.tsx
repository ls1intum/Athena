...
export default function BaseInfoHeader() {
  const { dataMode, viewMode, athenaUrl, athenaSecret, lmsUrl } = useBaseInfo();
  ...
  return (
      <div className="bg-white rounded-md p-4 mb-8">
          ...
          <label className="flex flex-col mt-4">
              <span className="text-lg font-bold">LMS Url</span>
              <p className="text-gray-500 mb-2">
                  This is the LMS Url used to distinguish between supported deployments. It&apos;s a compulsory field.
              </p>
              <input
                  className="border border-gray-300 rounded-md p-2"
                  value={lmsUrl}
                  onChange={(e) => dispatch({type: "SET_LMS_URL", payload: e.target.value})}
              />
          </label>
          <br/>
          <DataModeSelect
