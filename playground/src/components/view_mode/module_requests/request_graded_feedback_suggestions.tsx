...
export default function RequestGradedFeedbackSuggestions() {
  ...
  const {
    ...
    onError: (error) => {
      console.error(error);
      alert(
        `Failed to request graded feedback suggestions from Athena: ${error.message}. Is the URL correct?`
      );
    },
    onSuccess: () => {
      alert(`Graded feedback suggestions requested successfully!`);
    },
  });
  ...
  const responseSubmissionView = (response: ModuleResponse | undefined) => {
    ...
    return (
      submission && (
        <Disclosure
          title="Submission with Graded Feedback Suggestions"
          openedInitially
          className={{ root: "ml-2" }}
        >
  ...
  return (
    <div className="bg-white rounded-md p-4 mb-8">
      <h3 className="text-2xl font-bold mb-4">
        Request Graded Feedback Suggestions from Athena
      </h3>
      <p className="text-gray-500 mb-4">
        Request a list of graded feedback suggestions from Athena for the selected
      ...
      >
        {exercise && submission
          ? isLoading
            ? "Loading..."
            : "Request graded feedback suggestions"
          : exercise
