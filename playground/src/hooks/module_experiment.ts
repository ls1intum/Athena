...
type ModuleExperimentState = {
  ...
  sentEvaluationSubmissions: Map<number, Feedback[]>;
...
export default function useModuleExperiment(
  ...
  const [state, setState] = useState<ModuleExperimentState>({
    ...
    sentEvaluationSubmissions: new Map(),
  ...
  const [interactiveSelectedSubmissionsQueue, setInteractiveSelectedSubmissionsQueue] = useState<number[]>([]);
  ...
  const [
    interactiveSubmissionsToCatchUpWith,
    setInteractiveSubmissionsToCatchUpWith,
  ] = useState<number[]>([]);

  // Interactive mode: Feedbacks that need to be sent to Athena
  const [interactivePendingSubmissionsWithFeedback, setInteractivePendingSubmissionsWithFeedback] =
  ...
  // Interactive mode, step 1: set feedbacks for evaluation submissions so far (for all modules)
  const interactiveSetSubmissionsWithFeedbacks =
    (submissionsWithFeedbacks: { [submissionId: number]: Feedback[] }) => {
      if (experiment.executionMode !== "interactive") {
        return;
      }

      const sentSubmissions = new Set(state.sentEvaluationSubmissions.keys());
      const pendingSubmissionsWithFeedbacks = new Map(
        Object.entries(submissionsWithFeedbacks)
          .filter(
            ([submissionId]) => !sentSubmissions.has(Number(submissionId))
          )
          .map(([submissionId, feedbacks]) => [Number(submissionId), feedbacks])
      );

      setInteractivePendingSubmissionsWithFeedback(pendingSubmissionsWithFeedbacks);
    };

  // Interactive mode, step 2: send pending feedbacks to Athena (before doing submission selection with the selector module)
  const interactiveSendPendingSubmissionsWithFeedback = async () => {
    if (experiment.executionMode !== "interactive") {
      return;
    }

    const pendingSubmissonsWithFeedback = Array.from(interactivePendingSubmissionsWithFeedback.entries())
      .filter(([submissionId]) => !state.sentEvaluationSubmissions.has(submissionId))
    ...
    if (pendingSubmissonsWithFeedback.length === 0) {
      return;
    }

    for (const { submission, feedbacks } of pendingSubmissonsWithFeedback) {
      setInfo(
      ...
      try {
        ...
        setState({
          ...state,
          sentEvaluationSubmissions: new Map(state.sentEvaluationSubmissions.set(submission.id, feedbacks)),
        });
        setInteractivePendingSubmissionsWithFeedback(
          new Map(
            Array.from(interactivePendingSubmissionsWithFeedback.entries()).filter(
  ...
  // Interactive mode, step 3: request submission selection from the submission selector module
  const interactiveRequestSubmissionSelection = async () => {
    ...
    setInfo("Requesting submission selection...");
  ...
  // Interactive mode, step 4: add the selected submission to the queue (for all modules including the selector module)
  const interactiveAddSelectedSubmission = (
    submissionId: number,
    ...
    if (
      ...
      !interactiveSelectedSubmissionsQueue.includes(submissionId) &&
      !interactiveSubmissionsToCatchUpWith.includes(submissionId)
    ) {
      // Add submission to the queue
      setInteractiveSelectedSubmissionsQueue([
        ...interactiveSelectedSubmissionsQueue,
        submissionId,
      ]);
    }
    ...
    state.submissionsWithFeedbackSuggestions.forEach((_, submissionId) =>
      submissionIdsToIgnore.add(submissionId)
    );

    // Submissions that are skipped in the evaliation submissions
    const newSubmissionsToCatchUpWith = experiment.evaluationSubmissions
      .filter((submission) => !submissionIdsToIgnore.has(submission.id))
      .map((submission) => submission.id);

    setInteractiveSubmissionsToCatchUpWith([
      ...interactiveSubmissionsToCatchUpWith,
      ...newSubmissionsToCatchUpWith,
    ]);
  };

  // Interactive mode, step 5: select the next submission from the enqueued submissions
  const interactiveSelectNextSubmission = async () => {
    ...
    // First, try to pick from interactiveSubmissionsToCatchUpWith if there are any
    if (interactiveSubmissionsToCatchUpWith.length > 0) {
      try {
        const response = await requestSubmissionSelection.mutateAsync({
          exercise: experiment.exercise,
          submissions: experiment.evaluationSubmissions.filter((submission) =>
            interactiveSubmissionsToCatchUpWith.includes(submission.id)
          ),
        });

        let submissionId = response.data as number;
        if (submissionId === -1) {
          // Pick random submission from interactiveSubmissionsToCatchUpWith
          const randomIndex = Math.floor(
            Math.random() * interactiveSubmissionsToCatchUpWith.length
          );
          submissionId = interactiveSubmissionsToCatchUpWith[randomIndex];
        }

        setInteractiveSelectedSubmissionId(submissionId);
        setInteractiveSubmissionsToCatchUpWith([
          ...interactiveSubmissionsToCatchUpWith.filter(
            (id) => id !== submissionId
          ),
        ]);
        return;
    ...
    if (interactiveSelectedSubmissionsQueue.length > 0) {
      const submissionId = interactiveSelectedSubmissionsQueue[0];
      setInteractiveSelectedSubmissionId(submissionId);
      setInteractiveSelectedSubmissionsQueue([...interactiveSelectedSubmissionsQueue.slice(1)]);
      return;
    }
  ...
  // Interactive mode, step 6: Generate feedback suggestions for selected submission
  const stepGenerateFeedbackSuggestionsInteractive = () => {
    ...
    interactiveSendPendingSubmissionsWithFeedback().then(() => {
      const submission = experiment.evaluationSubmissions.find(
  ...
  return {
    state,
    info,
    interactiveAddSelectedSubmissionId: interactiveAddSelectedSubmission,
    interactiveRequestSubmissionSelection,
    interactiveSetSubmissionsWithFeedbacks:
    interactiveSetSubmissionsWithFeedbacks,
    interactiveSendPendingFeedbacks: interactiveSendPendingSubmissionsWithFeedback,
  };
}
